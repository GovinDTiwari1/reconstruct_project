...
def compute_hists(files,frames_dir,resize):
    imgs=[]
    for f in files:
        imgs.append(load_gray(os.path.join(frames_dir,f),resize))
    return imgs

def pairwise_sim(imgs):
    N=len(imgs)
...
def greedy_order(S):
    ...
    for _ in range(N):
        ...
        used[cur]=True
        row=S[cur].copy()
        row = np.where(np.array(used), -1.0, row)
        if np.all(row<=-1.0): break
        cur=int(np.argmax(row))
    if len(order)<N:
        for i in range(N):
            if not used[i]: order.append(i)
    return order

def total_score(order,S):
    s=0.0
    for i in range(len(order)-1):
        s+=S[order[i],order[i+1]]
    return s
...
def main():
    if len(sys.argv)<4:
        print("Usage: python fix_order.py <frames_dir> <num_frames> <out_list>")
        return
    ...
    if len(files)<N:
        N=len(files)
    files=files[:N]
    imgs=compute_hists(files,frames_dir,resize=(128,128))
    S=pairwise_sim(imgs)
    order=greedy_order(S)
    order_opt=two_opt(order,S,iter_limit=300)
    if total_score(order_opt,S) < total_score(order,S):
        order=order
    else:
        order=order_opt
    fs_score=total_score(order,S)
    rev_score=total_score(order[::-1],S)
    if rev_score>fs_score:
        order=order[::-1]
    out_dir='reordered'
